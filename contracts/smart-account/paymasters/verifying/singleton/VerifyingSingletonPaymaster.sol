...
import {BasePaymaster} from "@vechain/account-abstraction-contracts/core/BasePaymaster.sol";
 ...
 */
abstract contract VerifyingSingletonPaymaster is
    BasePaymaster,
    ReentrancyGuard,
    SingletonPaymasterErrors
{
    ...
    function depositFor(
        ...
        require(
            VTHO_TOKEN_CONTRACT.transferFrom(msg.sender, address(this), amount),
            "Paymaster deposit transfer failed"
        );
        paymasterIdBalances[paymasterId] =
            paymasterIdBalances[paymasterId] +
            amount;
        require(
            VTHO_TOKEN_CONTRACT.approve(address(entryPoint), amount),
            "Paymaster deposit approval failed"
        );
